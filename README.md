...
Take `kvsWebRTCClientMaster` as sample, add `RtcCertificate certificates[CERT_COUNT];` to **SampleConfiguration** in [Samples.h](samples/webrtc/Samples.h).
Then pass in the pre-generated certificate in initializePeerConnection() in [Common.c](samples/webrtc/Common.c).
...
The threadpool is enabled by default, and starts with 1 threads that it can increase up to 10 if all are actively in use. To change these values to better match the resources of your use case you can set the environment variables to do so:
