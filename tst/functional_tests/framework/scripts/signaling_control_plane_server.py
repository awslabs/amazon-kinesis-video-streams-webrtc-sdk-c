...
def index():
    ...
    metrics_file.write("Hi Anurag. This is metrics!")
    return 'welcome Anurag'
...
def setup_logging():
    logger_file_path = os.environ['SIGNALING_CP_SERVER_LOG_FILE']
    print("====>" + logger_file_path)
    global metrics_file
    metrics_file = open(os.environ['SIGNALING_CP_SERVER_METRICS_FILE'], "a")
    logging.basicConfig(filename = logger_file_path, level=logging.DEBUG, format = f'%(asctime)s %(levelname)s %(name)s %(threadName)s : %(message)s')