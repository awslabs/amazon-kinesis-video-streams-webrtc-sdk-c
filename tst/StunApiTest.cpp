...
TEST_F(StunApiTest, deserializeValidityTests)
{
    ...
    EXPECT_NE(STATUS_SUCCESS, deserializeStunPacket(pBuffer, SIZEOF(StunHeader) - 1, (PBYTE)TEST_STUN_PASSWORD, STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pStunPacket));
    ...
    EXPECT_EQ(STATUS_STUN_MESSAGE_INTEGRITY_MISMATCH, deserializeStunPacket(stunPacketBad, size, (PBYTE)TEST_STUN_PASSWORD, STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pStunPacket));
    MEMCPY(stunPacketBad, stunPacket, SIZEOF(stunPacket));
    // Modify the body bit
    ...
    EXPECT_EQ(STATUS_STUN_FINGERPRINT_MISMATCH, deserializeStunPacket(stunPacketBad, size, (PBYTE)TEST_STUN_PASSWORD, STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pStunPacket));

    // De-serialize and attempt to add to it
    EXPECT_EQ(STATUS_SUCCESS, deserializeStunPacket(pBuffer, size, (PBYTE)TEST_STUN_PASSWORD, STRLEN(TEST_STUN_PASSWORD) * SIZEOF(CHAR), &pStunPacket));
