...
TEST_F(SdpBufferSizeConfigTest, signalingMessageLenDerivedFromSdpLen)
{
    ...
    EXPECT_EQ((UINT32) KVS_SIGNALING_MESSAGE_LEN, MAX_SIGNALING_MESSAGE_LEN);
    UINT32 expectedSdpLen = ((UINT32) KVS_SIGNALING_MESSAGE_LEN - 1024) * 3 / 4;
...
// Test Case: Verify default values when KVS_SIGNALING_MESSAGE_LEN is not defined
TEST_F(SdpBufferSizeConfigTest, defaultBufferSizeValues)
{
#ifndef KVS_SIGNALING_MESSAGE_LEN
    EXPECT_EQ(25000u, MAX_SESSION_DESCRIPTION_INIT_SDP_LEN);
    EXPECT_EQ(18750u, MAX_SIGNALING_MESSAGE_LEN);
#else
    EXPECT_EQ((UINT32) KVS_SIGNALING_MESSAGE_LEN, MAX_SIGNALING_MESSAGE_LEN);
    EXPECT_EQ(((UINT32) KVS_SIGNALING_MESSAGE_LEN - 1024) * 3 / 4, MAX_SESSION_DESCRIPTION_INIT_SDP_LEN);
#endif
}
