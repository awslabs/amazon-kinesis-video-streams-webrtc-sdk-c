...
typedef struct {
    ...
    PStunPacket bindingResponseList[DEFAULT_NAT_TEST_MAX_BINDING_REQUEST_COUNT *
    	(NAT_BEHAVIOR_DISCOVER_PROCESS_TEST_COUNT * 2)];
...
STATUS executeNatTest(PStunPacket, PKvsIpAddress, PSocketConnection,
		UINT32, PNatTestData, PStunPacket *);

STATUS getMappAddressAttribute(PStunPacket, PStunAttributeAddress *);

STATUS discoverNatMappingBehavior(PIceServer, PNatTestData, PSocketConnection,
                              NAT_BEHAVIOR *);

STATUS discoverNatFilteringBehavior(PIceServer, PNatTestData, PSocketConnection,
                                NAT_BEHAVIOR *);
