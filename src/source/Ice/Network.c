...
BOOL isIpAddr(PCHAR hostname, UINT16 length)
{
    ...
    if (SSCANF(hostname, "%" SCNu32 ".%" SCNu32 ".%" SCNu32 ".%" SCNu32 "%n", &ip_1, &ip_2, &ip_3, &ip_4, &offset) == 4 &&
        ip_1 <= 255 && ip_2 <= 255 && ip_3 <= 255 && ip_4 <= 255 && (UINT32) offset == STRLEN(hostname)) {
        return TRUE;
    }
    ...
    offset = 0;
    if (SSCANF(hostname,
               "%" SCNx32 ":%" SCNx32 ":%" SCNx32 ":%" SCNx32 ":%" SCNx32 ":%" SCNx32 ":%" SCNx32 ":%" SCNx32 "%n",
               &ip_1, &ip_2, &ip_3, &ip_4, &ip_5, &ip_6, &ip_7, &ip_8, &offset) == 8 &&
        // Verify that the entire input was consumed - do not allow extra characters.
        (UINT32) offset == STRLEN(hostname) &&
