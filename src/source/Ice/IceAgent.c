...
STATUS iceAgentSendPacket(PIceAgent pIceAgent, PBYTE pBuffer, UINT32 bufferLen)
{
    ...
    // Cache the send target, then release the agent lock BEFORE the actual send.
    // pIceAgent->lock is a single coarse lock for the whole ICE subsystem and is
    // contended by RX incoming-data handling, connectivity checks and timers.
    // Holding it across iceUtilsSendData serializes every TX packet behind that
    // RX/ICE activity under bidirectional load. The selected pair's
    // socketConnection/turnConnection carry their own internal locks and
    // connection-closed guards, so the send is safe outside the agent lock.
    // destAddr is copied by value (KvsIpAddress is a small POD).
    pSocketConnection = pIceAgent->pDataSendingIceCandidatePair->local->pSocketConnection;
    destAddr = pIceAgent->pDataSendingIceCandidatePair->remote->ipAddress;

    MUTEX_UNLOCK(pIceAgent->lock);
    locked = FALSE;

    retStatus = iceUtilsSendData(pBuffer, bufferLen, &destAddr, pSocketConnection, pTurnConnection, isRelay);
    ...
    if (STATUS_FAILED(retStatus)) {
        ...
        if (retStatus == STATUS_SOCKET_CONNECTION_CLOSED_ALREADY) {
            ...
            // Re-read under the re-acquired lock: the selected pair may have changed
            // while the lock was released for the send.
