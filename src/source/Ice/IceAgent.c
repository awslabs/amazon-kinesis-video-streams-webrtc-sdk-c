    ...
    for (i = originalCount; i < pIceAgent->iceServersCount; i++) {
        if (pIceAgent->iceServers[i].isTurn) {
            ...
            if (pIceAgent->iceServers[i].transport == KVS_SOCKET_PROTOCOL_UDP ||
                ...
                retStatus = iceAgentInitRelayCandidate(pIceAgent, i, KVS_SOCKET_PROTOCOL_UDP, KVS_IP_FAMILY_TYPE_IPV4);
            ...
            if (pIceAgent->iceServers[i].transport == KVS_SOCKET_PROTOCOL_TCP ||
                pIceAgent->iceServers[i].transport == KVS_SOCKET_PROTOCOL_NONE) {
                retStatus = iceAgentInitRelayCandidate(pIceAgent, i, KVS_SOCKET_PROTOCOL_TCP, KVS_IP_FAMILY_TYPE_IPV4);
                if (STATUS_FAILED(retStatus)) {
...
STATUS iceCandidateSerialize(PIceCandidate pIceCandidate, PCHAR pOutputData, PUINT32 pOutputLength)
{
    ...
    if (IS_IPV4_ADDR(&(pIceCandidate->ipAddress))) {
        amountWritten = SNPRINTF(pOutputData, pOutputData == NULL ? 0 : *pOutputLength,
                                 "%u 1 udp %u %d.%d.%d.%d %d typ %s raddr 0.0.0.0 rport 0 generation 0 network-cost 999", (unsigned int)pIceCandidate->foundation,
                                 (unsigned int)pIceCandidate->priority, pIceCandidate->ipAddress.address[0], pIceCandidate->ipAddress.address[1],
        ...
        amountWritten = SNPRINTF(pOutputData, pOutputData == NULL ? 0 : *pOutputLength,
                                 ...
                                 (unsigned int)pIceCandidate->foundation, (unsigned int)pIceCandidate->priority, pIceCandidate->ipAddress.address[0],
