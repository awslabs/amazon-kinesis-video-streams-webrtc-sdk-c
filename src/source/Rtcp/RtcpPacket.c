...
STATUS rtcpNackListGet(PBYTE pPayload, UINT32 pPayloadLen, PUINT32 pSenderSsrc, PUINT32 pReceiverSsrc, PUINT16 pSequenceNumberList, PUINT32 pSequenceNumberListLen) {
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    ...
    CHK(pPayloadLen >= RTCP_NACK_LIST_LEN && (pPayloadLen % 4 == 0), STATUS_RTCP_INPUT_NACK_LIST_INVALID);

    *pSenderSsrc = getInt32(*(PUINT32) pPayload);
    *pReceiverSsrc = getInt32(*(PUINT32) (pPayload + 4));

    for (; i < pPayloadLen; i += 4) {
