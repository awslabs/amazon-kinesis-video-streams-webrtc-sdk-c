...
static RTC_RTP_TRANSCEIVER_DIRECTION getLocalDirectionFromRemoteMediaDescription(PSdpMediaDescription pMediaDescription)
{
    UINT32 i;
    RTC_RTP_TRANSCEIVER_DIRECTION remoteDirection;
    ...
    for (i = 0; i < pMediaDescription->mediaAttributesCount; i++) {
        if (STATUS_SUCCEEDED(parseTransceiverDirection(pMediaDescription->sdpAttributes[i].attributeName, &remoteDirection)) &&
            remoteDirection != RTC_RTP_TRANSCEIVER_DIRECTION_UNINITIALIZED) {
            // Invert: remote sendonly -> local recvonly, remote recvonly -> local sendonly
            if (remoteDirection == RTC_RTP_TRANSCEIVER_DIRECTION_SENDONLY) {
                return RTC_RTP_TRANSCEIVER_DIRECTION_RECVONLY;
            } else if (remoteDirection == RTC_RTP_TRANSCEIVER_DIRECTION_RECVONLY) {
                return RTC_RTP_TRANSCEIVER_DIRECTION_SENDONLY;
            }
            return remoteDirection; // sendrecv and inactive stay the same
        }
    }
    return RTC_RTP_TRANSCEIVER_DIRECTION_INACTIVE;
}
                                           ...
                                           PHashTable pUnknownCodecPayloadTypesTable, PHashTable pUnknownCodecRtpmapTable)
{
    ...
    if (avMlineCount != answerTransceiverCount) {
        DLOGW("Audio/video m-line count (%u) does not match answer transceiver count (%u)", avMlineCount, answerTransceiverCount);
    }
