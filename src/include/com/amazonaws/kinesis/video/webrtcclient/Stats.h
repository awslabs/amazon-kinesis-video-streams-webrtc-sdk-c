 ...
 */
typedef struct {
    CHAR dscp[MAX_STRING_LENGTH + 1]; //!< DSCP String
 ...
 */
typedef struct {
    CHAR url[MAX_STRING_LENGTH + 1]; //!< STUN/TURN server URL
    CHAR protocol[MAX_STRING_LENGTH + 1]; //!< Valid values: UDP, TCP
...
typedef struct {
    CHAR url[MAX_STRING_LENGTH + 1]; //!< For local candidates this is the URL of the ICE server from which the candidate was obtained
    CHAR transportId[MAX_STRING_LENGTH + 1]; //!< ID of object that was inspected for RTCTransportStats
    BYTE address[MAX_ADDRESS_LENGTH + 1]; //!< IPv4 or IPv6 address of the candidate
 ...
 */
typedef struct {
    CHAR rtcpTransportStatsId[MAX_STRING_LENGTH + 1]; //!< ID of the transport that gives stats for the RTCP component
    CHAR selectedCandidatePairId[MAX_STRING_LENGTH + 1]; //!< ID of the object inspected to produce RtcIceCandidatePairStats
    CHAR localCertificateId[MAX_STRING_LENGTH + 1]; //!< For components where DTLS is negotiated, give local certificate
    CHAR remoteCertificateId[MAX_STRING_LENGTH + 1]; //!< For components where DTLS is negotiated, give remote certificate
...
typedef struct {
    ...
    CHAR trackId[MAX_STRING_LENGTH + 1]; //!< ID representing current track attached to the sender of the stream
    CHAR mediaSourceId[MAX_STRING_LENGTH + 1]; //!< ID representing the current media source
    CHAR senderId[MAX_STRING_LENGTH + 1]; //!< The stats ID used to look up the RTCAudioSenderStats or RTCVideoSenderStats object sending this stream
    CHAR remoteId[MAX_STRING_LENGTH + 1]; //!< ID to look up the remote RTCRemoteInboundRtpStreamStats object for the same SSRC
    CHAR rid[MAX_STRING_LENGTH + 1]; //!< Exposes the rid encoding parameter of this RTP stream if it has been set, otherwise it is undefined
    CHAR encoderImplementation[MAX_STRING_LENGTH + 1]; //!< Identifies the encoder implementation used.
    ...
    PQualityLimitationDurationsRecord qualityLimitationDurations; //!< Total time (seconds) spent in each reason state
 ...
 */
typedef struct {
    CHAR localId[MAX_STRING_LENGTH + 1]; //!< Used to look up RTCOutboundRtpStreamStats for the SSRC
...
typedef struct {
    ...
    SignalingClientStats signalingClientStats; //!< Signaling Client stats object
