...
// Forward declarations for ESP implementation
...
STATUS terminateConnectionWithStatusEsp(PSignalingClient pSignalingClient, SERVICE_CALL_RESULT serviceCallResult);
                           ...
                           PAwsCredentialProvider pCredentialProvider, PSignalingClient* ppSignalingClient)
{
    ...
    // Initialize ESP WebSocket implementation
    // We're not using LWS-specific structures anymore
    ...
    // Initialize ESP WebSocket client context (specific to ESP implementation)
    // This is placeholder to indicate we would set up ESP WebSocket here
    // Actual initialization happens when connecting
...
STATUS freeSignaling(PSignalingClient* ppSignalingClient)
{
    ...
    // Free ESP WebSocket client if exists
    // Instead of lws_context_destroy, we'll use ESP WebSocket cleanup
    // This is a placeholder - the actual implementation would be in LwsApiCallsESP.c
    if (pSignalingClient->pWebsocketContext != NULL) {
        MUTEX_LOCK(pSignalingClient->lwsServiceLock);
        // Cleanup ESP WebSocket here (handled in LwsApiCallsESP.c)
        // The cleanup function would be called here
...
STATUS signalingSendMessageSync(PSignalingClient pSignalingClient, PSignalingMessage pSignalingMessage)
{
    ...
    // Perform the call - use ESP implementation instead of LWS
    CHK_STATUS(sendEspSignalingMessage(pSignalingClient, pSignalingMessage));
...
STATUS describeChannel(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        if (apiCall) {
            ...
            if (STATUS_SUCCEEDED(retStatus)) {
                // Use ESP implementation instead of LWS
                retStatus = describeChannelEsp(pSignalingClient, time);
...
STATUS createChannel(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        // Use ESP implementation instead of LWS
        retStatus = createChannelEsp(pSignalingClient, time);
...
STATUS getChannelEndpoint(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        if (apiCall) {
            ...
            if (STATUS_SUCCEEDED(retStatus)) {
                // Use ESP implementation instead of LWS
                retStatus = getChannelEndpointEsp(pSignalingClient, time);
...
STATUS getIceConfig(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        // Use ESP implementation instead of LWS
        retStatus = getIceConfigEsp(pSignalingClient, time);
...
STATUS deleteChannel(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        // Use ESP implementation instead of LWS
        retStatus = deleteChannelEsp(pSignalingClient, time);
...
STATUS connectSignalingChannel(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        ...
        if (!ATOMIC_LOAD_BOOL(&pSignalingClient->connected)) {
            ATOMIC_STORE(&pSignalingClient->result, (SIZE_T) SERVICE_CALL_RESULT_NOT_SET);
            // Use ESP implementation instead of LWS
            retStatus = connectSignalingChannelEsp(pSignalingClient, time);
...
STATUS joinStorageSession(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        if (ATOMIC_LOAD_BOOL(&pSignalingClient->connected)) {
            // Use ESP implementation instead of LWS
            retStatus = joinStorageSessionEsp(pSignalingClient, time);
...
STATUS describeMediaStorageConf(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (STATUS_SUCCEEDED(retStatus)) {
        if (apiCall) {
            ...
            if (STATUS_SUCCEEDED(retStatus)) {
                // Use ESP implementation instead of LWS
                retStatus = describeMediaStorageConfEsp(pSignalingClient, time);
                // Store the last call time on success
                if (STATUS_SUCCEEDED(retStatus)) {
