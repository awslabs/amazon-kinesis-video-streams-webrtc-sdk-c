...
#if 0
typedef struct {
    KvsSignalingClientData *pClientData;
    PReceivedSignalingMessage pReceivedSignalingMessage;
} message_received_cb_task_args_t;

static void message_received_cb_task(void *arg)
{
    message_received_cb_task_args_t *pArgs = (message_received_cb_task_args_t *)arg;
    KvsSignalingClientData *pClientData = pArgs->pClientData;
    PReceivedSignalingMessage pReceivedSignalingMessage = pArgs->pReceivedSignalingMessage;
    ...
    pClientData->onMessageReceived((UINT64)pClientData->customData, pReceivedSignalingMessage);
    MEMFREE(pArgs);
}
#endif
...
STATUS createCredentialProvider(KvsSignalingClientData *pClientData)
{
...
#if 0
    // Try to get credentials to extract region from token
    retStatus = pClientData->pCredentialProvider->getCredentialsFn(
        pClientData->pCredentialProvider->customData, &pCredentials);

    if (STATUS_SUCCEEDED(retStatus) && pCredentials != NULL) {
        credentialsAcquired = TRUE;

        // Extract region from session token if available
        if (pCredentials->sessionToken != NULL && pCredentials->sessionToken[0] != '\0') {
            char region[32] = {0};
            if (STATUS_SUCCEEDED(extractRegionFromCredentialToken(pCredentials->sessionToken, region, sizeof(region))) &&
                region[0] != '\0') {
                // Update the region in the channel info
                if (pClientData->channelInfo.pRegion != NULL) {
                    ESP_LOGI(TAG, "Updating region from '%s' to '%s' based on token",
                          pClientData->channelInfo.pRegion, region);

                    // Allocate memory for the new region string
                    PCHAR pNewRegion = MEMALLOC(strlen(region) + 1);
                    if (pNewRegion != NULL) {
                        strcpy(pNewRegion, region);

                        // Replace the region in the channel info
                        // Note: We're not freeing the old region string as it might be a static string
                        pClientData->channelInfo.pRegion = pNewRegion;
                    }
                }
            }
        }
    ...
    // Reset status after region extraction attempt
...
/**
 * @brief Create KVS signaling client with configuration
 */
STATUS createKvsSignalingClientWithConfig(PKvsSignalingConfig pConfig, PVOID *ppSignalingClient)
{
    STATUS retStatus = STATUS_SUCCESS;
    KvsSignalingClientData *pClientData = NULL;
...
#if 0
/**
 * @brief Extract region from AWS credential token
 *
 * @param token The AWS credential token
 * @param region Buffer to store the extracted region
...
STATUS extractRegionFromCredentialToken(const char* token, char* region, size_t region_size)
{
    ...
    CHK(token != NULL && region != NULL && region_size > 0, STATUS_NULL_ARG);

    // Initialize region buffer
    region[0] = '\0';

    // Find the region part in the token
    for (i = 0; token[i] != '\0'; i++) {
        if (token[i] == '.' && token[i+1] != '\0') {
            // Check for "region=" after the dot
            if (strncmp(&token[i+1], "region=", 7) == 0) {
                i += 8; // Skip past "region="

                // Copy the region value
                for (j = 0; j < region_size - 1 && token[i+j] != '\0' && token[i+j] != '.' && token[i+j] != '&'; j++) {
                    region[j] = token[i+j];
                }
