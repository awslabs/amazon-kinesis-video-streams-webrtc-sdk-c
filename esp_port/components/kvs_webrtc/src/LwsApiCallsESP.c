...
// SSL verification option for development
// Comment out in production environments
#define CONFIG_SKIP_COMMON_NAME_CHECK
...
static void esp_websocket_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    ESP_LOGI(TAG, "WebSocket event: %d, signalingClient: %p", (int)event_id, pSignalingClient);
...
STATUS connectEspSignalingClient(PSignalingClient pSignalingClient)
{
    ...
    ws_cfg.skip_cert_common_name_check = true;   // Skip certificate verification
#else
    ws_cfg.skip_cert_common_name_check = false;  // Verify certificates
#endif
                          ...
                          PCHAR* pResponseData, PUINT32 pResponseLen)
{
    ...
    response = (PCHAR) MEMALLOC(HTTP_RESPONSE_BUFFER_SIZE);
    ...
    config.buffer_size_tx = 2 * HTTP_RESPONSE_BUFFER_SIZE;

    // Configure SSL verification settings
    // For AWS services, we need to disable common name verification
    config.skip_cert_common_name_check = true;

    // For development, set skip_cert_common_name_check to true to skip cert verification
    // In production, proper certs should be provided
#ifdef CONFIG_SKIP_COMMON_NAME_CHECK
    config.skip_cert_common_name_check = true;   // Skip certificate verification
#else
    config.skip_cert_common_name_check = false;  // Verify certificates
#endif

    // Use the Amazon Root CA certificate
    config.cert_pem = amazon_root_ca;
    // config.cert_len = strlen(amazon_root_ca);
