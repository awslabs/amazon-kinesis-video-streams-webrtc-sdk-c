...
{
    STATUS retStatus = STATUS_SUCCESS;
    GstElement *appsrcAudio = NULL, *receiverPipeline = NULL;
    GstBus* bus;
    GstMessage* msg;
    GError* error = NULL;
    ...
    receiverPipeline = gst_parse_launch(audioVideoDescription, &error);

    appsrcAudio = gst_bin_get_by_name(GST_BIN(receiverPipeline), "appsrc-audio");
    ...
    CHK_ERR(receiverPipeline != NULL, STATUS_INTERNAL_ERROR, "[KVS Gstreamer Master] Pipeline is NULL");

    gst_element_set_state(receiverPipeline, GST_STATE_PLAYING);

    /* block until error or EOS */
    bus = gst_element_get_bus(receiverPipeline);
    msg = gst_bus_timed_pop_filtered(bus, GST_CLOCK_TIME_NONE, GST_MESSAGE_ERROR | GST_MESSAGE_EOS);
    ...
    if (receiverPipeline != NULL) {
        gst_element_set_state(receiverPipeline, GST_STATE_NULL);
        gst_object_unref(receiverPipeline);
    }
    if (appsrcAudio != NULL) {
